...
## [2025-11-09] - 🧾 توسيع إيصال القبض بملخص الدفعات

### الملفات المعدلة
- `kitchen_factory/app.py` (السطور 3486-3830، 3846-4205): إضافة ملخص تفصيلي للعربون والدفعات في إيصال الـ PDF، وتمرير السجل الكامل للدفعات مع كل عملية طباعة.

### الهدف
عرض العربون وجميع الدفعات مع تواريخها داخل الإيصال لضمان شفافية أعلى وتتبع مالي أدق للعميل والإدارة.

### التأثير
- ✅ يوفر جدولاً يوضح نوع كل دفعة وتاريخها ومبلغها داخل الإيصال.
- ✅ يضيف إجماليات العربون والدفعات الأخرى وإجمالي المدفوع لسهولة المراجعة.
- ✅ يحافظ على منطق حساب المتبقي دون الحاجة لتعديلات على قاعدة البيانات.

## [2025-11-09] - ✅ منح مسؤول العمليات صلاحيات إدارة المواد

### الملفات المعدلة
- `kitchen_factory/app.py` (السطور 31-6295): تعريف ثابت للأدوار المسؤولة عن إدارة المواد، وتحديث جميع عمليات إضافة المواد والمخزون ونقص المواد لاستخدامه.
- `kitchen_factory/templates/order_materials.html` (السطور 72-220): السماح لمسؤول العمليات برؤية نموذج إضافة المواد، تلوين نصوص عمودي النواقص والحالة (أخضر عند الاكتمال، أصفر لما دون ذلك)، وجعل نص عمود النسبة أسود عند كون الاكتمال أقل من 100٪.

### الهدف
تمكين دور مسؤول العمليات من إضافة المواد وإدارة المخزون أسوة بالمدير ومسؤول المخزن مع توحيد التحقق من الصلاحيات.

### التأثير
- ✅ مسؤول العمليات يستطيع الآن إضافة مواد جديدة من واجهة المخزون.
- ✅ تم توحيد التحقق من الصلاحيات عبر جميع مسارات إدارة المواد.
- ✅ تقليل تكرار القوائم الثابتة للصلاحيات في الكود.
...
- `kitchen_factory/templates/admin_tools.html` (السطور 30-160): إضافة إعداد اختيار حجم الإيصال وبطاقة لإدارة المخازن ضمن أدوات المشرف.
- `database_schema.md` (السطور 680-760): توثيق الحقول الجديدة والخصائص المحسوبة.
- `kitchen_factory/templates/admin_warehouses.html` (جديد): قائمة تفاعلية للمخازن مع إجراءات التفعيل والتعيين الافتراضي.
- `kitchen_factory/templates/admin_warehouse_form.html` (جديد): نموذج إنشاء/تعديل مخزن.
- `kitchen_factory/app.py` (السطور 3350-3805، 3718-4188، 4070-4188، 4927-4985، 6100-6400): دعم اختيار حجم الورقة (A4/A5)، عرض مربعات تحديد لطريقة الدفع، منح مدير العمليات صلاحية تقرير الفنيين، إجبار تحديث الصفحة بعد إضافة دفعة، وتخزين الإعداد من أدوات المشرف بالإضافة لمسارات إدارة المخازن.
