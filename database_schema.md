...
- `pricing_notes` - ملاحظات التسعير (مصدر السعر أو تفاصيل التعديل)
- `is_paid` - هل تم الدفع؟ (افتراضي: false)
- `paid_at` - تاريخ الدفع
- `payment_id` - معرف الدفعة (FK إلى TechnicianPayment)
- `calculated_at` - تاريخ ووقت احتساب المستحق (UTC)
...
- `rate_source` - مصدر السعر المعتمد (default_rate، manual_override، missing_default)
...
#### الخصائص المحسوبة:
- `manufacturing_total_cost` - التكلفة الإجمالية للتصنيع (سعر المتر × الأمتار)
- `installation_total_cost` - التكلفة الإجمالية للتركيب (سعر المتر × الأمتار)
